...
return {
  ...
  apply: function (ctx) {
    ...
    var nonce = 0
    // 危险命令的一次性确认令牌:命中 ask 时签发,模型经 ask_user_question 征询用户后
    // 携 confirmToken + choice 重跑确认。仅一次性有效,防模型绕过(需先拦截签发+用户明确选择)。
    ...
    function decide(s, pendingId, action) {
      ...
      if (action === 'always') {
        return addServerAllowRule(s.server, cmd).then(function () {
        ...
        return ensureSession(server).then(function (disp) {
          ...
          return waitLive(disp, 20000, exec && exec.signal).then(function (ready) {
            ...
            if (v.action === 'ask') {
              ...
              var choice = String((args && args.choice) || '')
              // 首次拦截:签发一次性确认令牌,返回 blocked。模型据此调用 ask_user_question
              // (已验证可用的原生确认卡)征询用户,再携 confirmToken + choice 重跑确认执行。
              // 不再在本工具内尝试 userQuestions.ask —— 宿主平面动态工具分发 agent 作用域
              // Question 瀑布无法映射到当前会话,会 fail closed;改由 ask_user_question 承担弹卡。
              if (confirmToken === undefined) {
                var tkey = 'ct' + (++nonce)
                askTokens.set(tkey, { serverId: server.id, cmd: cmd, at: Date.now() })
                disp.outAll += '$ ' + cmd + NL
                ...
                return {
                  ...
                  message: '命令命中危险规则「' + askRule + '」,需要用户确认后才执行。请先调用 ask_user_question 弹出确认卡询问用户,再把用户的选择映射为 choice(执行一次=allow,永久放行该命令=always,拒绝=deny),并用返回的 confirmToken 重新调用本工具(netshell_run)执行。'
                }
              }
              // 二次调用:校验令牌(一次性,限于该服务器+该命令,10 分钟内有效)。
              var pend = askTokens.get(confirmToken)
              if (!pend || Date.now() - pend.at > 600000) {
                askTokens.delete(confirmToken)
                disp.outAll += '$ ' + cmd + NL + '[已拒绝] 确认令牌无效或已过期' + NL
                pushEvent(disp, { type: 'deny', command: cmd, rule: askRule })
                return { ok: false, blocked: true, action: 'deny', rule: '确认令牌无效或已过期', output: '', command: cmd }
              }
              askTokens.delete(confirmToken)
              if (pend.serverId !== server.id || pend.cmd !== cmd) {
                disp.outAll += '$ ' + cmd + NL + '[已拒绝] 确认令牌与命令不匹配' + NL
                pushEvent(disp, { type: 'deny', command: cmd, rule: askRule })
                return { ok: false, blocked: true, action: 'deny', rule: '确认令牌与命令不匹配', output: '', command: cmd }
              }
              if (choice === 'deny') {
                disp.outAll += '$ ' + cmd + NL + '[已拒绝] ' + askRule + NL
                pushEvent(disp, { type: 'deny', command: cmd, rule: askRule })
                return { ok: false, blocked: true, action: 'deny', rule: askRule, output: '', command: cmd }
              }
              if (choice === 'always') {
                return addServerAllowRule(server, cmd).then(function () {
                  disp.outAll += '$ ' + cmd + NL + '[永久放行] ' + cmd + NL
              ...
              if (choice === 'allow') {
                disp.outAll += '$ ' + cmd + NL + '[放行] ' + cmd + NL
                pushEvent(disp, { type: 'ask-allow', command: cmd })
                return runRemote(server, cmd, timeoutMs, exec && exec.signal).then(function (r) { return appendOut(r, false) })
              }
              // 未知 choice 一律按拒绝处理(安全默认)。
              disp.outAll += '$ ' + cmd + NL + '[已拒绝] 未知选择,默认拒绝' + NL
              pushEvent(disp, { type: 'deny', command: cmd, rule: askRule })
              return { ok: false, blocked: true, action: 'deny', rule: '未知选择,默认拒绝', output: '', command: cmd }
            }
...
harness.handle('netshell.input', function (args) {
    ...
    var runTool = harness.defineTool({
      name: 'netshell_run',
      description: '在指定的远程服务器(NetShell 终端)上执行一条 shell 命令并返回输出。命令通过真实 SSH 终端执行,支持权限控制:命中 deny 规则(如 rm -rf /)直接拦截。命中 ask 规则(如 rm -rf *)时返回 blocked + confirmToken,此时你必须先调用 ask_user_question 弹出原生确认卡询问用户,用户选择后把选择映射为 choice(执行一次=allow,永久放行该命令=always,拒绝=deny),再用返回的 confirmToken 调用本工具确认执行;未带 confirmToken 的重复调用只会再次返回 blocked。用 netshell_servers 查询服务器 id 作为 server 参数。',
      parameters: {
        ...
        confirmToken: { type: 'string', description: '命中 ask 规则首次拦截时返回的确认令牌;用户经 ask_user_question 明确选择后再带此令牌重跑确认执行' },
        choice: { type: 'string', description: '用户经 ask_user_question 的选择:allow=执行一次,always=永久放行该命令,deny=拒绝' }
      },
      output: {
        schema: { type: 'json' },
        render: function (args, value) {
