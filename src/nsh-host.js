...
// 工具 ask 挂起(面板裁决)与 confirmToken 的共同有效期:到期自动撤销挂起并作废令牌。
var TOOL_ASK_TTL = 600000
...
var BUILTIN_RULES = [
  ...
  { pattern: 'rm *', action: 'ask', note: '删除文件/目录' },
...
return {
  ...
  apply: function (ctx) {
    ...
    var nonce = 0
    // 危险命令的工具挂起记录(askTokens):首选路径是插件直调宿主 userQuestions.ask
    // 在对话窗口弹原生确认卡、原地等待真人回答(机制性绑定,无令牌);仅当弹卡不可用
    // (无 answerer / 子代理上下文 / 动态加载无服务)时回退「面板挂起 + 一次性令牌」——
    // 令牌只有在面板真实裁决(allow/always/deny)后才可兑现,choice 参数不作授权依据。
    var askTokens = new Map()
    // 模型建连确认的在途请求:同一 Agent + 服务器同时只允许一条确认/建连流程,
    // 避免并发工具调用在用户回答前各自启动 SSH;底层 SSH 会话仍由所有 Agent 共享。
    var modelConnectJobs = new Map()
    // 模型授权按 Agent + 服务器保留在当前插件生命周期内;只有用户主动断开才清除。
    // SSH 异常退出、网络断开和连接等待超时只会使当前会话失效,不会撤销这项授权。
    var modelAuthorizedServers = new Map()

    function modelAgentId(agent) {
      return agent && agent.id != null ? String(agent.id) : null
    }

    function modelAuthorizationKey(agent, serverId) {
      var agentId = modelAgentId(agent)
      return agentId == null ? null : agentId + '\u0000' + String(serverId)
    }

    function hasModelAuthorization(agent, serverId) {
      var users = modelAuthorizedServers.get(String(serverId))
      var agentId = modelAgentId(agent)
      return !!(users && agentId != null && users.has(agentId))
    }

    function grantModelAuthorization(agent, serverId) {
      var agentId = modelAgentId(agent)
      if (agentId == null) return
      var key = String(serverId)
      var users = modelAuthorizedServers.get(key)
      if (!users) { users = new Set(); modelAuthorizedServers.set(key, users) }
      users.add(agentId)
    }

    function revokeModelAuthorizations(serverId) {
      modelAuthorizedServers.delete(String(serverId))
    }
    ...
    function submitEnter(s) {
      ...
      if (v.action === 'allow') {
        ...
        nonce += 1
        // from: 'line' = 人工在终端敲的行(裁决放行后向 PTY 写 \r 执行);
        // from: 'tool' = 模型经 netshell_run 挂起(裁决只记账,执行走独立 ssh -T)。
        s.pending = { id: 'pd' + nonce, command: cmd, rule: v.rule && (v.rule.note || v.rule.pattern), from: 'line' }
    ...
    // 防绕过:netshell.input 的 data 若内嵌 \r / \n,必须拆段送入——每一段回车都
    // 走 onInput('\r') → submitEnter → evaluateFor,保证不存在「绕过 Guard 直达
    // PTY」的输入形态(0.6.0 及之前,一条多字符 data 可整体旁路 Guard 直接执行)。
    // 合法浏览器客户端只发单键(回车是单独一条 '\r'),含换行的多字符只可能来自
    // 粘贴等批量输入,拆段语义与逐键等价。
    function feedMultiline(s, data) {
      var segs = data.replace(/\r\n/g, '\r').split(/[\r\n]/)
      for (var i = 0; i < segs.length; i++) {
        var seg = segs[i]
        for (var j = 0; j < seg.length; j++) onInput(s, seg.charAt(j))
        if (i < segs.length - 1) onInput(s, '\r')
      }
    }

    function addServerAllowRule(server, cmd) {
      return credentials.modifyRecord(PKEY, function (cur) {
        ...
        for (var i = 0; i < arr.length; i++) {
          if (arr[i] && arr[i].id === server.id) {
            var rules = Array.isArray(arr[i].rules) ? arr[i].rules : []
            for (var j = 0; j < rules.length; j++) if (rules[j] && rules[j].pattern === cmd) return cur
            return hostBlank(refFor(server.id)).then(function (rule) {
              rule.pattern = cmd
              rule.action = 'allow'
              rule.note = '用户永久放行'
              rules.push(rule)
    ...
    function findDecided(serverId, cmd) {
      var found = null
      askTokens.forEach(function (entry, key) {
        if (!found && entry.decision && entry.serverId === serverId && entry.cmd === cmd) {
          found = { key: key, entry: entry }
        }
      })
      return found
    }
    ...
    function panelAskFallback(disp, server, cmd, askRule, diag) {
      ...
      nonce += 1
      var pid = 'pd' + nonce
      askTokens.set(tkey, { serverId: server.id, cmd: cmd, at: Date.now(), pendingId: pid, decision: null })
      disp.pending = { id: pid, command: cmd, rule: askRule, from: 'tool', token: tkey }
      pushEvent(disp, { type: 'ask', command: cmd, rule: askRule })
      // 与令牌同窗自动过期:防止模型弃走后挂起永久阻塞该会话输入。
      timer.timeout(TOOL_ASK_TTL).then(function () {
        var entry = askTokens.get(tkey)
        if (entry && entry.pendingId === pid) {
          askTokens.delete(tkey)
          if (disp.pending && disp.pending.id === pid) cancelPending(disp, cmd)
        }
      }, function () {})
      return Promise.resolve({
    ...
    function decide(s, pendingId, action) {
      ...
      var from = s.pending.from || 'line'
      var token = s.pending.token || null
      s.pending = null
      var canWrite = !!s.handle
      var hasHist = Array.isArray(s.hist)
      // 工具挂起(from 'tool')的裁决记到 askTokens 令牌上,作为 netshell_run 兑现的
      // 唯一授权凭证;不向 PTY 写任何字节——它从未占用行缓冲,执行走独立 ssh -T。
      // 人工挂起(from 'line')保持原行为:\r 执行 / \u0015 复位远端行。
      if (token) {
        var entry = askTokens.get(token)
        if (entry) entry.decision = action
      }
      if (action === 'deny') {
        if (canWrite && from === 'line') write(s, '\u0015')
        pushEvent(s, { type: 'deny', command: cmd, rule: '用户拒绝' })
        return Promise.resolve({ ok: true })
      }
      var after = function () {
        if (canWrite && from === 'line') write(s, '\r')
        if (hasHist && from === 'line') s.hist.push(cmd)
        pushEvent(s, { type: 'ask-allow', command: cmd })
        return Promise.resolve({ ok: true })
      }
      if (action === 'always') {
        return addServerAllowRule(s.server, cmd).then(after)
      }
      return after()
    }
...
function makeAskpass(s) {
    ...
    function resolveLocalShell() {
      var candidates = [
        { name: 'bash', kind: 'posix' },
        { name: 'sh', kind: 'posix' },
        { name: 'pwsh', kind: 'powershell' },
        { name: 'powershell', kind: 'powershell' },
        { name: 'cmd', kind: 'cmd' }
      ]
      function next(i) {
        if (i >= candidates.length) return Promise.reject(new Error('找不到本地 Shell(bash/sh/pwsh/powershell/cmd)'))
        var candidate = candidates[i]
        return subprocess.resolveExecutable(candidate.name).then(function (path) {
    ...
    function spawnLocalSession() {
      return resolveLocalShell().then(function (shell) {
        var s = {
          id: 'ns' + Date.now().toString(36) + (++nonce),
          server: { id: 'local', name: '本地终端', local: true }, status: 'connecting',
          outAll: '', outBase: 0, dropped: 0, events: [], evSeq: 0, line: '', hist: [], histIdx: undefined,
          pending: null, tail: '', atPwPrompt: false, hint: null, closedReason: null, askpassPath: null
        }
        var argv = shell.kind === 'cmd'
          ? [shell.path]
          : shell.kind === 'powershell'
            ? [shell.path, '-NoLogo', '-NoExit']
            : [shell.path, '-i']
        return subprocess.spawnTerminal({
          argv: argv, cwd: '.', env: { TERM: 'xterm-256color' }, rows: ROWS, cols: COLS, graceMs: 3000
        }).then(function (handle) {
          s.handle = handle
          sessions.set(s.id, s)
          var outputIterator = handle.output[Symbol.asyncIterator]()
    ...
    function findOpenSession(serverId) {
      var found = null
      sessions.forEach(function (s) {
        if (!found && s.server && s.server.id === serverId && s.status !== 'closed') found = s
      })
      return found
    }

    function ensureSession(server) {
      var existing = findOpenSession(server.id)
      if (existing) return Promise.resolve(existing)
      return spawnSession(server)
    }

    function ensureModelSession(server, cmd, exec) {
      var agent = exec && exec.agent
      var jobKey = modelAuthorizationKey(agent, server.id) || String(server.id)
      var active = modelConnectJobs.get(jobKey)
      if (active) return active

      var job = Promise.resolve().then(function () {
        var approval = hasModelAuthorization(agent, server.id)
        ...
        return approval.then(function (decision) {
          ...
          // 授权只对当前 Agent 生效,底层 SSH 会话仍由所有 Agent 共享。
          grantModelAuthorization(agent, server.id)
      ...
      modelConnectJobs.set(jobKey, job)
      var clear = function () { if (modelConnectJobs.get(jobKey) === job) modelConnectJobs.delete(jobKey) }
    ...
    function cancelPending(s, cmd) {
      if (s.pending) {
        ...
        s.pending = null
        // 只有「人工敲入的行」才需要向远端发 \u0015 清掉已回写的命令;工具挂起从未
        // 碰过 PTY 行缓冲,清行反而会误伤用户可能正在输入的内容。
        if (from === 'line' && s.handle) write(s, '\u0015')
    ...
    function toolRunExecute(args, exec) {
      ...
      return resolveServer(serverId).then(function (server) {
        ...
        return ensureModelSession(server, cmd, exec).then(function (connection) {
          ...
          return waitLive(disp, 20000, exec && exec.signal).then(function (ready) {
            ...
            if (v.action === 'ask') {
              ...
              disp.outAll += '$ ' + cmd + NL
              pushEvent(disp, { type: 'ask', command: cmd, rule: askRule })
              // 授权统一入口:kind 为 'allow' | 'always'。授权只能来自真实的人工裁决
              // (确认卡答案 / 面板点击);模型的 choice 参数不再作为授权依据。
              var confirmedRun = function (kind) {
                var run = function () {
                  disp.outAll += (kind === 'always' ? '[永久放行] ' : '[放行] ') + cmd + NL
                  pushEvent(disp, { type: 'ask-allow', command: cmd })
                  return runRemote(server, cmd, timeoutMs, exec && exec.signal).then(function (r) { return appendOut(r, false) })
                }
                if (kind === 'always') return addServerAllowRule(server, cmd).then(run)
                return run()
              }
              var deniedResult = function (reason) {
                disp.outAll += '[已拒绝] ' + reason + NL
                pushEvent(disp, { type: 'deny', command: cmd, rule: reason })
                return { ok: false, blocked: true, action: 'deny', rule: reason, output: '', command: cmd }
              }
              // 路径一:模型携 confirmToken 重跑(面板回退路径的兑现)。裁决只认面板记账,
              // 令牌一次性、限于该服务器+该命令、TOOL_ASK_TTL 内有效;未裁决不消耗令牌。
              var confirmToken = args && args.confirmToken
              if (confirmToken !== undefined) {
                var pend = askTokens.get(confirmToken)
                if (!pend || Date.now() - pend.at > TOOL_ASK_TTL) {
                  askTokens.delete(confirmToken)
                  return deniedResult('确认令牌无效或已过期')
                }
                if (pend.serverId !== server.id || pend.cmd !== cmd) {
                  askTokens.delete(confirmToken)
                  return deniedResult('确认令牌与命令不匹配')
                }
                if (!pend.decision) {
                  return {
                    ok: false, blocked: true, action: 'ask', needsConfirmation: true,
                    confirmToken: confirmToken, rule: askRule, command: cmd, output: '',
                    message: '用户尚未在「远程终端」面板裁决(执行一次 / 永久放行该命令 / 拒绝),confirmToken 仍有效。请提醒用户完成面板裁决后再重跑。'
                  }
                }
                askTokens.delete(confirmToken)
                if (pend.decision === 'deny') return deniedResult(askRule)
                return confirmedRun(pend.decision)
              }
              // 路径二:面板已裁决但模型漏带令牌重跑 —— (服务器, 命令) 精确匹配直接兑现。
              var decided = findDecided(server.id, cmd)
              if (decided) {
                askTokens.delete(decided.key)
                if (decided.entry.decision === 'deny') return deniedResult(askRule)
                return confirmedRun(decided.entry.decision)
              }
              // 同一会话同时只允许一条挂起,避免覆盖与双重确认。
              if (disp.pending) {
                return {
                  ok: false, blocked: true, action: 'ask', rule: askRule, command: cmd, output: '',
        ...
        return toolBody().then(function (v) { disp.agentBusy = false; return v }, function (e) { disp.agentBusy = false; throw e })
      })
      })
    }
    ...
    harness.handle('netshell.local.connect', function () {
      return spawnLocalSession().then(function (s) { return { id: s.id, pid: s.handle.pid } })
    })

    harness.handle('netshell.sessions.list', function () {
      var list = []
      sessions.forEach(function (s) {
        list.push({ id: s.id, serverName: s.server ? s.server.name : 'unknown', status: s.status, pending: s.pending ? s.pending.id : null, agentBusy: !!s.agentBusy, closedReason: s.closedReason || null, hint: s.hint || null })
      })
...
harness.handle('netshell.input', function (args) {
      ...
      if (typeof data === 'string' && data.length > 0 && data.length <= 4096) {
        // 单键 / ESC 序列保持原路径;内嵌 \r 或 \n 的多字符一律拆段过 Guard。
        if (data.length === 1 || (data.indexOf('\r') === -1 && data.indexOf('\n') === -1)) onInput(s, data)
        else feedMultiline(s, data)
      }
      return Promise.resolve({ ok: true })
    })
    ...
    harness.handle('netshell.poll', function (args) {
      var s = sessions.get(args && args.id)
      // 会话已在宿主侧消失(断开/移除)时返回良性结果而不是 reject:
      // 客户端可能仍残留该 id 一轮轮询,reject 会被网关记成 handler 失败刷屏。
      if (!s) {
        return Promise.resolve({
          status: 'closed', output: '', lossy: false, dropped: 0, nextCursor: 0,
          events: [], pending: null, hint: null, closedReason: '会话不存在', gone: true,
          atPwPrompt: false, cols: COLS, rows: ROWS
        })
      }
      var output = s.outAll
    ...
    harness.handle('netshell.disconnect', function (args) {
      ...
      if (serverId) {
       ...
       revokeModelAuthorizations(serverId)
      }
    ...
    var runTool = harness.defineTool({
      ...
      parameters: {
        ...
        confirmToken: { type: 'string', description: '仅弹卡不可用的回退路径使用:用户在「远程终端」面板裁决后,携此令牌重跑以兑现;正常弹卡路径无需传' },
        choice: { type: 'string', description: '已废弃:授权以确认卡/面板的真实裁决为准,本参数被忽略' }
      },
      output: {
        schema: { type: 'json' },
        render: function (args, value) {
