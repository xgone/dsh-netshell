...
模型首次使用某台服务器，或用户手动断开该服务器后再次使用时，`netshell_run` 会先弹出远程连接确认；确认前不会启动 SSH。确认在当前插件生命周期内按服务器保留，网络故障、SSH 异常退出或连接等待超时只会触发自动重连，不会再次询问。GUI 中已有的人工连接不会替代模型确认；确认服务不可用时插件会拒绝自动建连。
